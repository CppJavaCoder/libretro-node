...
void Call(int index, const F& f)
{
    ...
    cb_func->ts.BlockingCall([index, f](Napi::Env env, Napi::Function callback) {
        try {
            ...
            Logger::Log(LogCategory::Fatal, "Retro binding", fmt::format("Caught unhandled exception in callback '{}'. {}", s_cb_names[index], e.what()));
            std::exit(EXIT_FAILURE);
        }
        catch (...) {
            Logger::Log(LogCategory::Fatal, "Retro binding", fmt::format("Caught unhandled exception in callback '{}'", s_cb_names[index]));
            std::exit(EXIT_FAILURE);
        }
    });
}
...
Napi::Value Startup(const Napi::CallbackInfo& info)
{
    return SafeCall(info.Env(), [&info]() {
        ...
        ::Frontend::StartInfo start_info{
            ...
            icon_data,
            AsStrUtf8(obj.Get("libretro")),
            AsStrUtf8(obj.Get("configDir")),
            AsStrUtf8(obj.Get("dataDir"))
        };
...
Napi::Value On(const Napi::CallbackInfo& info)
{
    return SafeCall(info.Env(), [&info]() {
        ...
        if (cb_func) {
            Logger::Log(LogCategory::Warn, "Retro binding", fmt::format("Overriding callback '{}'. `{}` -> `{}`", s_cb_names[index],
