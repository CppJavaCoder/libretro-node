#include "addon/retro/config_section.h"
    ...
    Napi::ObjectWrap<Section>{info}
{
    SafeCall(info.Env(), [this, &info]() {
        m_section = std::make_unique<::RETRO::Core::ConfigSection>(GetCore().ConfigOpenSection(AsStrUtf8(info[0])));
