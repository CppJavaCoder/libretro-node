...
void ImGuiCtx::InitContext(SDL::Window& window)
{
...
void ImGuiCtx::InitBackends(SDL::Window& window)
{
    if (!ImGui_ImplSDL2_InitForOpenGL(window.Get(), NULL))
...
void ImGuiCtx::Initialize(SDL::Window& window)
{
    if (m_init)
        Deinitialize();
