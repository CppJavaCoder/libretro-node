...
Edit::Edit(RETRO::Core& core, RETRO::Cheat::Block& block,
    ...
    const std::vector<std::string>& values)
{
    ...
    for (std::size_t i{}; i < addresses.size(); ++i) {
        ...
        RETRO::Cheat::retro_cheat_code code;
...
{
    std::map<std::string, RETRO::Cheat::Entry>* entries =
...
void Edit::Update()
{
    ...
    if (m_codes.empty() && m_options.empty()) {
        ...
        bool found = std::find_if(m_entry->codes.begin(), m_entry->codes.end(),
            [](const RETRO::Cheat::retro_cheat_code& v) { return v.value == -1; }) != m_entry->codes.end();
